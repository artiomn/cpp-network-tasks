...
socket_wrapper::Socket accept_client(socket_wrapper::Socket &server_sock) {
  ...
  std::cout << "Client from "
            << inet_ntop(
                   AF_INET,
                   &(reinterpret_cast<const sockaddr_in *const>(&client_addr)
                         ->sin_addr),
                   &addr[0], addr.size())
            << "..." << std::endl;
  return client_sock;
...
class Transceiver {
public:
  Transceiver(socket_wrapper::Socket &&client_sock)
...
public:
  ...
  std::string get_request() {
    ...
    while (true) {
      auto result =
          recv(client_sock_, &buffer[recv_bytes], size - recv_bytes, 0);
      ...
      auto ret_iter =
          std::find_if(fragment_begin, fragment_begin + result,
                       [](char sym) { return '\n' == sym || '\r' == sym; });
    ...
    auto input_str = std::string(buffer.begin(), buffer.begin() + recv_bytes);
    if (*(input_str.end() - 1) != ' ')
      input_str += ' ';
    std::string::size_type name_length = input_str.find(' ');
    auto result = input_str.substr(0, name_length);
    std::string::size_type bytes_length = input_str.find(' ', name_length + 1);
    if (bytes_length == std::string::npos) {
      ...
      if (bytes_for_transmission_length == std::string::npos) {
        std::cout << "Size for transmission is not defined" << std::endl;
        bytes_for_transmission_ = 0;
        bytes_shift_ = 0;
      } else {
        bytes_for_transmission_ = std::stoi(input_str.substr(
            bytes_length + 1, bytes_for_transmission_length - bytes_length));
      }
    }
    std::cout << result << ' ' << bytes_shift_ << ' ' << bytes_for_transmission_
              << std::endl;
...
class Client {
public:
  Client(socket_wrapper::Socket &&sock) : tsr_(std::move(sock)) {
...
int main(int argc, const char *const argv[]) {
  ...
  try {
    ...
    socket_wrapper::Socket server_sock = {
    ...
    while (true) {
      ...
      pending_tasks.push_back(std::async(
          ...
          [&](socket_wrapper::Socket &&sock) {
            ...
            std::cout << "Client tid = " << std::this_thread::get_id()
                      << std::endl;
            auto result = client.process();
            std::cout << "Client with tid = " << std::this_thread::get_id()
                      << " exiting..." << std::endl;
    ...
    std::cerr << e.what() << " [" << sock_wrap.get_last_error_string() << "]!"
              << std::endl;
    return EXIT_FAILURE;
  }
